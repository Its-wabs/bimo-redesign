...
export default function BimoButton({ 
  ...
  const onMouseLeave = contextSafe(() => {
    ...
    exitTlRef.current.to(waveContainerRef.current, {
      y: "-130%",
      duration: 0.5,
      ease: "power2.in",
    }, 0);
    ...
    exitTlRef.current.to(buttonRef.current, {
  ...
  return (
    <div 
      ...
      className={`group relative flex items-center cursor-pointer pointer-events-auto select-none overflow-hidden rounded-2xl active:shadow-none active:translate-y-1 transition-all ${className}`}
    >
      {/* THE WAVY LIQUID LAYER */}
      <div 
        ref={waveContainerRef}
        className="absolute inset-0 z-20 -translate-y-[130%] pointer-events-none w-full h-[150%]"
      >
        <div className="absolute top-0 left-0 w-full h-full bg-[#3D1E12]" />
        <svg 
          className="absolute bottom-0 left-0 w-full h-[30%] translate-y-[95%]"
          viewBox="0 0 1440 320" 
          preserveAspectRatio="none"
        >
          <path fill="#3D1E12" d="M0,160 C120,320 240,0 360,160 C480,320 600,0 720,160 C840,320 960,0 1080,160 C1200,320 1320,0 1440,160 L1440,0 L0,0 Z" />
        </svg>
      </div>

      {/* BUTTON TEXT (Uses Layout Props) */}
      <div className={`relative z-30 ${paddingX} ${paddingY} flex items-center justify-center w-full`}>
        <span 
          ref={textRef}
          style={ {
            color : textColor,
          }}
          className={`font-english uppercase hover:text-[#FDF6E9] tracking-tighter whitespace-nowrap will-change-transform ${fontSize}`}
        >
          {text}
        </span>
      </div>
